...
class ChessClient(QObject):
    ...
    def errorOccurred(self, error):
        if error == QTcpSocket.ConnectionRefusedError:
            ...
            self.mainWindow.errorLabel.setText("The server is not running! Starting the server...")
            self.mainWindow.errorLabel.setStyleSheet("color:rgb(0, 170, 0)")

            # Launch server
            self.serverThread = ServerThread(self.ip, self.port, self.mainWindow)
        ...
        while self.socket.bytesAvailable() > 0:
            ...
            if data.startswith("set_nick:"):
                ...
                if self.playerNick == "light":
                    self.mainWindow.errorLabel.setText(f"Wait for another player... Invite your friend! Network: {self.ip.toString()}:{self.port}")
                    self.mainWindow.errorLabel.setStyleSheet("color:rgb(0, 170, 0)")
                ...
                sanMove = self.mainWindow.board.logic.coordsToSAN(startX, startY, newX, newY, promotionPiece)
                ...
                self.mainWindow.netActivePlayer = "light" if self.mainWindow.netActivePlayer == "dark" else "dark"
